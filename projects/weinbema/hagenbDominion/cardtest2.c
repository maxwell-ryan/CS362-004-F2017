...
int main(int argc, char** argv) {
    ...
    struct gameState manipulatedControlGame;
    ...
    for (x = 0; x < controlGame.deckCount[currentPlayer]; x++) {
      //printf("deck cards number %d\n", controlGame.deck[currentPlayer][x]);
      if (controlGame.deck[currentPlayer][x] == 4) {
        controlGame.deck[currentPlayer][x] = 5;
      }
    }
    for (x = 0; x < controlGame.handCount[currentPlayer]; x++) {
      //printf("hand cards number %d\n", controlGame.hand[currentPlayer][x]);
      if (controlGame.hand[currentPlayer][x] == 4) {
        controlGame.hand[currentPlayer][x] = 5;
      }
    }
    ...
    for(x = 0; x < manipulatedControlGame.discardCount[currentPlayer]; x++) {
            manipulatedControlGame.discard[currentPlayer][x] = silver;
    }
